...
function mountScrollWorld(container, config) {
  // ponytail: 재진입 가드만 둡니다. StrictMode는 이펙트를 두 번 돌리는데, 두 번째 마운트가
  // 씬과 카피를 한 벌 더 만들면 개발 화면이 통째로 어긋납니다. 이 엔진은 전역 리스너를
  // 거두는 destroy가 없지만, 컨테이너가 페이지 수명 내내 안 사라지므로 실제로 새지는
  // 않습니다. 월드를 언마운트하는 라우트가 생기면 그때 teardown을 붙이세요.
  if (container.dataset.swMounted) return;
  ...
  // 카피 레이어는 서버가 미리 그려 둡니다(components/World.jsx). 여기서 새로 만들면
  // 같은 글이 DOM에 두 벌 생기고, 무엇보다 JS를 안 돌리는 크롤러에게는 이 다섯 장면의
  // 본문 — 이력서의 알맹이 — 이 통째로 안 보입니다. 있으면 그대로 씁니다.
  const copylayer = container.querySelector(':scope > .sw-copylayer') || el('div', 'sw-copylayer');
  ...
  const copies = [], dots = [];
  SECTIONS.forEach((s, i) => {
    const pre = copylayer.children[i];
    const c = pre && pre.classList.contains('sw-copy') ? pre : el('article', 'sw-copy');
    ...
    // 라벨 span은 아래에서 opacity:0으로 눕혀 두므로 접근성 트리에서는 이름 없는 버튼이 됩니다.
    // 같은 라벨로 여기서 직접 이름을 답니다 — 마운트 뒤에 밖에서 덧칠할 일이 없도록.
    if (s.label) dot.setAttribute('aria-label', `${s.label} 장면으로 이동`);
