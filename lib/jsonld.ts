...
import { REPOS, SHARES, MEDIA_WORK, isoDate } from './content';
...
// 영상·오디오. Media가 화면에 그리는 그 목록 그대로입니다.
const media = itemList('media-work', '영상 · 오디오 작업', MEDIA_WORK, (w) => ({
  '@type': 'CreativeWork',
  name: w.title,
  description: w.note,
  about: w.where,
  temporalCoverage: w.when.replace(/\s/g, ''),
  url: abs('/#media'),
  author: { '@id': abs('/#person') },
}));
