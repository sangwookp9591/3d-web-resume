...
강점은 화면을 잘 만드는 쪽보다 FRONT ↔ ADMIN ↔ BACK의 계약을 맞추는 쪽에 있습니다.
그 계약을 말로만 두지 않고 실제 테스트와 운영 문서까지 연결해 뒀습니다.
남긴 것도 기능 개수보다는 팀의 작업 방식 쪽입니다.
...
카카오 인앱 브라우저에서 외부 브라우저로 넘어가는 OAuth 크로스 컨텍스트 복귀는 세션에 담지 않았습니다.
state에 redirect를 동봉해 stateless로 풀었습니다 — 세션에 기대면 브라우저가 바뀌는 순간 잃어버립니다.`,
  },
  {
    id: 'test',
...
정본을 서버 한 곳에 뒀습니다. 복구 상태 머신은 페이지에서 떼어 독립 모듈(cart-restore-flow)로 뺐습니다 —
상태 전이가 컴포넌트에 붙어 있으면 테스트할 수 있는 단위가 아닙니다.
...
세분 모델을 적용했습니다 — 마이그레이션 리스크를 줄이려고요.
...
StyleX 디자인 토큰은 codemod 스크립트(tokens:check/fix)로 자동 교정했습니다 — 리뷰가 아니라 도구로 집행하려고요.
...
빨라지는 것과 터졌을 때 번지는 범위는 같이 봐야 합니다.
...
트리거 발급은 동기 호출 대신 ShedLock 분산락 기반 Outbox 워커로 돌립니다 — 유실과 중복을 구조에서 막으려고요(멀티 인스턴스 안전).
...
무료가 언제 유료로 바뀌는지를 빼면 그건 공유가 아니라 청구서입니다.
...
API 응답 지연·타임아웃으로 이어지고 502/500과 커넥션 풀 고갈로 번진 이력입니다.
...
정리하면 새로 나온 것을 팀이 오늘 바로 쓸 수 있는 형태로 바꿔 놓는 쪽입니다.`,
  },
  {
    id: 'aiwork',
...
AGENTS.md를 단일 규칙원으로 두고 CLAUDE.md는 포인터로만 남겼습니다 — 규칙은 복제되는 순간부터 어긋납니다.
